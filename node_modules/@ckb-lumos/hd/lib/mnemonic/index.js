...
var _word_list = _interopRequireDefault(require("./word_list"));
...
if (_word_list.default.length !== RADIX) {
  throw new Error(`Word list should have ${RADIX} words, but ${_word_list.default.length} received in fact`);
}
...
function deriveChecksumBits(entropyBuffer) {
  ...
  const hash = _crypto.default.createHash("sha256").update(entropyBuffer).digest();
...
function mnemonicToSeedSync(mnemonic = "", password = "") {
  ...
  return _crypto.default.pbkdf2Sync(mnemonicBuffer, saltBuffer, PBKDF2_ROUNDS, KEY_LEN, "sha512");
}
...
function mnemonicToSeed(mnemonic = "", password = "") {
  return new Promise((resolve, reject) => {
    try {
      ...
      _crypto.default.pbkdf2(mnemonicBuffer, saltBuffer, PBKDF2_ROUNDS, KEY_LEN, "sha512", (err, data) => {
        if (err) {
          reject(err);
        }
...
function mnemonicToEntropy(mnemonic = "") {
  ...
  const bits = words.map(word => {
    const index = _word_list.default.indexOf(word);
...
function entropyToMnemonic(entropyStr) {
  ...
  const words = chunks.map(binary => {
    ...
    return _word_list.default[index];
  });
  return words.join(" ");
}
...
function generateMnemonic() {
  ...
  const entropy = "0x" + _crypto.default.randomBytes(entropySize).toString("hex");
